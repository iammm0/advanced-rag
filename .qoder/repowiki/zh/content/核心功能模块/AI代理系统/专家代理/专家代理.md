...
## 简介
本文件面向“专家代理系统”的技术文档，围绕多专家协同工作流展开，系统性阐述各专家代理的职责边界、输入输出规范、提示词设计、实现细节与性能优化策略，并提供专家代理选择与组合使用的最佳实践。专家代理体系包括但不限于：概念解释专家、文档检索专家、摘要生成专家、批判性分析专家、代码分析专家、公式分析专家、科学编码专家、示例生成专家、练习生成专家等。
...
### 代码分析专家
- 职责：提取代码的语法树与语义信息，识别函数、类、导入、变量与关键字，估算复杂度。
- 输入：代码文本（可自动识别语言）。
- 输出：包含语言、函数、类、导入、变量、关键字、行数与复杂度的分析结果。
- 实现细节：正则匹配与关键字集合识别；复杂度估算基于行数、控制结构与函数数量。

```mermaid
flowchart TD
Start(["开始"]) --> Detect["检测代码语言"]
Detect --> ExtractF["提取函数定义"]
Detect --> ExtractC["提取类定义"]
Detect --> ExtractI["提取导入语句"]
ExtractF --> Vars["提取变量名"]
ExtractC --> Vars
ExtractI --> Vars
Vars --> KW["提取关键字"]
...
### 公式分析专家
- 职责：提取公式中的变量、关系与函数，分析结构与复杂度。
- 输入：LaTeX 格式公式字符串。
- 输出：变量列表、关系列表、函数列表、结构信息与公式原文。
- 实现细节：正则匹配变量、关系与函数；结构分析包括是否方程、是否含分数/根号/积分/求和/矩阵等；复杂度按运算符、函数、分数、根号计数。
...
### 科学编码专家、示例生成专家、练习生成专家
- 职责：生成代码解决方案、提供具体实例与学习题目。
- 输入：task（问题/需求）、context（可选）。
- 输出：完成结果，包含内容、来源（如有）与置信度。
- 实现细节：继承基类，使用系统提示词与 LLM 生成；可结合 RAG 检索增强事实性与上下文相关性。
...
## 结论
专家代理系统通过“基类抽象 + 专家子类 + 工作流编排 + 检索与生成服务”的分层设计，实现了多专家协同与高效内容生成。文档检索专家与批判性分析专家通过 RAG 检索确保事实性与可追溯性；概念解释专家、摘要生成专家与代码/公式分析专家分别覆盖知识普及、信息归纳与专业内容解析。配合流式输出与并行检索，系统在性能与交互体验上取得平衡。建议在实际部署中结合业务场景选择合适的专家组合，并通过配置中心动态调整模型与检索策略。

## 附录
- 专家代理选择与组合最佳实践
  - 通用场景：先用文档检索专家获取上下文，再用摘要生成专家进行归纳，最后用批判性分析专家验证。
  - 专业场景：结合代码分析专家与公式分析专家，针对技术内容提供结构化解析。

...
## 项目结构
项目采用分层与功能域结合的组织方式：
- 表现层（前端 Next.js）：位于 web/ 目录，负责用户界面与交互。
...
- 前端 Next.js：负责用户界面渲染与交互，通过 HTTP/Server-Sent Events 与后端通信。
...
Chunk --> KG["知识抽取Neo4j"]
KG --> Embed["向量化嵌入服务"]
Embed --> StoreMongo["存储到 MongoDBchunks"]

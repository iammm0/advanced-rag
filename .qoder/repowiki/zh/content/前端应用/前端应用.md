...
- 应用入口与全局布局
  ...
  - 首页用于初始化与自动跳转至聊天页。
- 页面级路由
  - 聊天页：核心对话与 RAG/深度研究流程。
  - 文档页：知识空间与文档管理、上传与分页展示。
- 组件层
  - UI 基础组件：布局、导航、加载进度、Toast 等。
  - 聊天组件：消息渲染、侧边栏、深度研究渲染器等。
  - 文档组件：单/批量上传、进度与状态轮询。
- 类型与工具
  - 类型定义：聊天消息、对话、知识空间、模型等。
...
- 组件负责 UI 表达与交互（消息、侧边栏、上传、布局等）。
- 类型定义保证前后端契约清晰（聊天消息、对话、知识空间、模型等）。
...
章节来源
- [web/lib/api.ts:62-86](file://web/lib/api.ts#L62-L86)
- [web/app/chat/page.tsx:317-340](file://web/app/chat/page.tsx#L317-L340)
- [web/app/documents/page.tsx:118-127](file://web/app/documents/page.tsx#L118-L127)

## 结论
advanced-rag 前端应用以 Next.js App Router 为基础，采用模块化组件与类型驱动的开发方式，围绕聊天与知识库两大核心场景构建。通过统一的 API 客户端与主题上下文，实现了良好的可维护性与一致性。聊天页在状态持久化、流式响应、RAG/深度研究等方面具备较强工程化能力；文档页在知识空间管理与上传流程上提供了完整的用户体验。建议持续关注性能优化与国际化扩展，以进一步提升可用性与全球化支持。
...
章节来源
- [web/components/chat/ChatMessage.tsx:9-25](file://web/components/chat/ChatMessage.tsx#L9-L25)
- [web/components/chat/ChatSidebar.tsx:15-21](file://web/components/chat/ChatSidebar.tsx#L15-L21)
- [web/components/document/BatchDocumentUpload.tsx:7-17](file://web/components/document/BatchDocumentUpload.tsx#L7-L17)
...
### 数据模型（概要）
- ChatMessage
  - 字段：message_id、role、content、timestamp、sources、recommended_resources、recommended_users、user_relationships、recommendation_reason、cypher_queries
- Conversation
...
- [web/types/conversation.ts:1-10](file://web/types/conversation.ts#L1-L10)
- [web/lib/api.ts:27-104](file://web/lib/api.ts#L27-L104)
...
章节来源
- [web/lib/api.ts:54-86](file://web/lib/api.ts#L54-L86)
- [web/lib/api.ts:106-345](file://web/lib/api.ts#L106-L345)
...
- [web/lib/theme.tsx:15-111](file://web/lib/theme.tsx#L15-L111)
